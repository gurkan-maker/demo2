...
def generate_flow_vs_dp_graph(scenario, valve, op_point, details, req_cv):
    ...
    # Create plot with smooth curve using spline interpolation
    if len(dp_range) > 3 and len(flow_rates) > 3:
        # Create spline interpolation for smooth curve
        x_smooth = np.linspace(min_dp, max_dp, 200)
        spline = make_interp_spline(dp_range, flow_rates, k=3)
...
def plot_cv_curve_matplotlib(valve, op_points, req_cvs, theoretical_cvs, scenario_names):
    ...
    spline = make_interp_spline(openings, cv_values, k=3)
    y_smooth = spline(x_smooth)
