...
# ========================
class EnhancedPDFReport(FPDF, HTMLMixin):
    def __init__(self, logo_bytes=None, logo_type=None):
        ...
        self.secondary_color = (230, 240, 255)  # Light blue
        self.accent_color = (220, 53, 69)  # Red for warnings
...
# ========================
def create_smooth_cv_curve(valve):
...
def plot_cv_curve_matplotlib(valve, op_points, req_cvs, theoretical_cvs, scenario_names):
    ...
    # Create smooth curve for valve Cv
    x_smooth, y_smooth = create_smooth_cv_curve(valve)
    plt.plot(x_smooth, y_smooth, 'b-', linewidth=2.5, label='Valve Cv', zorder=1)
    
    # Operating points
    for i, op in enumerate(op_points):
        actual_cv = valve.get_cv_at_opening(op)
        plt.plot(op, actual_cv, 'ro', markersize=8, zorder=4)
        plt.annotate(f'S{i+1}', (op + 1.5, actual_cv), 
                     fontsize=10, color='red', weight='bold', zorder=5)
    
    # Required Cv lines
    for i, cv in enumerate(req_cvs):
        plt.axhline(y=cv, color='r', linestyle='--', linewidth=1.5, zorder=2)
        plt.text(102, cv, f'Corrected S{i+1}: {cv:.1f}', 
                 fontsize=9, color='red', ha='right', va='center', zorder=3)
    
    # Theoretical Cv lines
    for i, cv in enumerate(theoretical_cvs):
        plt.axhline(y=cv, color='g', linestyle=':', linewidth=1.5, zorder=2)
        plt.text(102, cv, f'Theoretical S{i+1}: {cv:.1f}', 
                 fontsize=9, color='green', ha='right', va='center', zorder=3)
    
    # Professional styling
    plt.title(f'{valve.size}" Valve Cv Characteristic', fontsize=14, fontweight='bold')
    plt.xlabel('Opening Percentage (%)', fontsize=12)
...
def generate_flow_vs_dp_graph(scenario, valve, op_point, details, req_cv):
    ...
    dp_range = np.linspace(min_dp, max_dp, 100)
    flow_rates = []
    ...
    if len(dp_range) > 3 and len(flow_rates) > 3:
        x_smooth = np.linspace(min(dp_range), max(dp_range), 300)
...
def plot_cv_curve(valve, op_points, req_cvs, theoretical_cvs, scenario_names):
    ...
    fig.update_yaxes(range=[0, max(y_smooth) * 1.1])
    return fig
...
def main():
            ...
            for i, scenario in enumerate(scenarios):
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        st.image(flow_fig, use_column_width=True)
    ...
    if export_btn:
        if not st.session_state.results:
            ...
            st.stop()
        try:
            # Generate smooth curve plot for PDF
